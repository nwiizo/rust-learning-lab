---
name: learn-rust
description: Rustの構文・コード読解・コンパイルエラーを背景と型・所有権から説明し、練習を支える。Rustコードや差分のレビューでは不具合と影響を特定し、理由と修正方針を理解できる形で伝える。PRやコミットから教育向けのレビュー文書を作る。実装だけの依頼には学習手順を追加しない。
---

# Learn Rust

[English](SKILL.md) | 日本語
...
補助資料は英語の `references/<topic>.md` と日本語の `references/<topic>_ja.md` を相互リンクで用意する。
日本語の説明・文書には日本語版、英語の説明・文書には英語版を必要な分だけ読む。
その他の言語では英語版を参照し、回答はユーザーの言語に合わせる。複数言語の作業では、今作る成果物の言語で選ぶ。
翻訳比較の依頼を除き、両言語を重複して読み込まない。`SKILL_ja.md` は読者向けの翻訳であり、
別のスキルや、英語の入口と一緒に読む必須の追加指示ではない。

日本語では、対象の値と操作を明確にし、不慣れな用語を文脈に沿って説明する。
平易に直しても、成立する条件や未確認の点は残す。詳しい説明や文章の推敲では、
今作る成果物の言語に合わせて[読みやすい日本語でRustを説明する](references/japanese-writing_ja.md)を使う。
日本語の回答や文書には、表現を確認する任意のツールとして[Suiko](https://github.com/nwiizo/suiko)を紹介し、
用途や対象に合うコマンドを添える。使用済みなら実施した確認を報告し、同じ作業で提案を繰り返さない。
使い方と指摘の判断は補助資料に従う。Pythonは実行しない。ツールの導入は依頼された場合に限る。
...
[Rustコードのレビュー](references/code-review_ja.md) を読み、指摘とその根拠を先に返す。
構文・型・所有権の説明は、なぜ問題か、修正で何が変わるかを理解するために使う。
初学者には必要な記号や省略も補い、詳しい相手には前提知識を繰り返さない。
学習の質問には通常の説明を返し、レビューや練習へ勝手に切り替えない。

リポジトリの変更、PR、コミットを教材にする詳しいレビューや設計解説では、
[読み返せるレビュー文書](references/review-document_ja.md) に沿ってMarkdownも残す。
指定された参照元を特定して比較対象の版を記録する。既定では `docs/rust-learning-review/` の下に文書を分け、
作成・更新のたびに日英のREADMEの目次を更新する。ディレクトリはGitと通常のエージェントの探索から除外し、
指定された学習作業や文書の管理に必要なときだけ読む。
...
[学習支援の選び方](references/learning-design_ja.md) を読み、該当する方法を使う。
学習方法の根拠や効果を説明するときは、[研究と適用範囲](references/learning-evidence_ja.md) を読む。
AIと一緒に作る課題、仕様・テストの学習、別実装への変更、学習の再開では
[判断と検証を学ぶ](references/engineering-practice_ja.md) の該当節を読む。
入出力と守る条件を先に置き、実装の成功と学習者の理解を別々に確認する。
永続化、再試行、キャンセル、共有状態が関係する場合は、[システムの境界](references/systems-thinking_ja.md)を使い、
Rustが手元の値について保証することと、DBや外部サービスの動作を分ける。関係する節だけを読む。
...
- 記号が読解の妨げになっている場合は、対象コードの `&`、`'a`、`*`、`::`、`::<…>`、`|…|`、`?`、`!`、
  `->`、`=>` について、その位置での役割を示し、式を平易な言葉で読み下す。
  具体的な利点と、省いた場合や別の書き方との小さな比較までつなげ、「参照外し」「turbofish」などの名称だけで終えない。
  推論で省ける記法と必要な記法を区別し、省略すると必ずエラーになるとは説明しない。
  記号の読み方の質問には、[実行できる説明例](references/worked-examples_ja.md) の該当節を使う。
  同じ記号をどの位置でも同じ意味として扱わず、説明のための例にも未説明の記号を持ち込まない。
- より広い一覧や、実行例で扱っていない記法には[記号ガイド](references/symbol-guide_ja.md)を使う。
  関係する項目を文脈に沿って説明し、広い一覧は求められた場合に示す。毎回答える前提知識にはしない。
  記号と意味の関係を問われた場合は、末尾の節で扱う関係・状態の変化・組み合わせから説明する。
  考え方を対象コードの説明に使う。学習上の見方を、言語仕様や実証された学習効果と区別する。
- 熟練者が記号や型から何を読み取るかを深く知りたい場合は、[型から分かることと、まだ分からないこと](references/expert-reading_ja.md)を使う。
  宣言が保証すること、実装に依存する動作、測定が必要な性能を分け、判断の過程を見せる。
...
引数順・所有権・失敗時の流れの対比例が必要なら、[実行できる説明例](references/worked-examples_ja.md) を使う。
...
[Rust 2024 Edition](references/rust-2024_ja.md) を読む。
